...
class predictive_frame_slack:
    ...
    def __init__(self, scenario_num, robot_type, x0, dt, tf, U_max, V_max, alpha_values, beta_values, num_constraints_hard, x_r_list, wpt_radius, t_list, reward_list, obstacle_list, if_disturb, disturb_std, disturb_max):
        ...
        if robot_type == 'SingleIntegrator2D':
            ...
            self.slack_hard = cp.Variable((self.num_constraints_hard,1))
            self.const2 = [self.A1_hard @ self.u1 <= self.b1_hard + self.slack_hard, self.A1_soft @ self.u1 <= self.b1_soft + cp.multiply(self.alpha_soft, self.v) + self.slack_soft, \
                      self.alpha_soft >= np.zeros((self.num_constraints_soft)), 
                      self.slack_soft >= np.zeros((self.num_constraints_soft)), cp.norm2(self.u1) <= self.U_max]
            self.objective2 = cp.Minimize(cp.sum_squares(self.u1) + cp.sum_squares(self.alpha_soft-self.alpha_1_param) 
                                          + cp.sum_squares(self.slack_soft))
            self.relaxed_controller = cp.Problem(self.objective2, self.const2)
        else:
            # Define constrained Optimization Problem
            self.u1 = cp.Variable((2,1))
            #self.u1_ref = cp.Parameter((2,1), value = np.zeros((2,1)))
    ...
    def forward(self, x0, curr_t, sub_list):
        ...
        if flag == "fail":
            ...
            slack_sum = sum(abs(self.slack_hard.value)) + sum(abs(self.slack_soft.value))
            reward = 0
            i -= 1
        ...
        return x_list, t_list, flag, slack_sum, reward, curr_wpt
...
def fitness_score_slack(comb, x0, curr_t, pred_frame):
    ...
    if (len(sub_list)>0):
        x_list, t_list, flag, slack_sum, reward, curr_wpt = pred_frame.forward(x0, curr_t, sub_list)
        if flag == "success":
    ...
    score = (pred_frame.reward_max-reward)*reward_weight + slack_sum
...
def genetic_comb_slack(x0, curr_time, pred_frame):
    ...
    for i in range(num_comb):
        ...
        traj, score, reward, curr_wpt = fitness_score_slack(comb_appended, x0, curr_time, pred_frame)
...
def deterministic_slack(x0, curr_time, pred_frame):
    
    num_states = len(pred_frame.x_r_list)
    init_comb = np.ones(shape=(num_states))
    traj, r, reward, curr_wpt = fitness_score_slack(init_comb, x0, curr_time, pred_frame)
    comb_best = init_comb
    traj_best = traj
    r_min = r
    reward_best = reward
    ...
    while traj_best == {}:
        ...
        for i in range(num_states):
            if i!=num_states-1 and dropped_constraints.get(i)==None:
                ...
                temp_comb[i] = 0
                traj, r, reward, curr_wpt= fitness_score_slack(temp_comb, x0, curr_time, pred_frame)
                if traj_best == {}:
                    if r < r_min:
                        r_min = r
                        candidate_idx = i
