...
slack_constraints_soft = cp.Variable((num_constraints_soft1,1))
slack_constraints_clf = cp.Variable((num_constraints_clf,1))
const1 = [A1_hard @ u1 <= b1_hard, A1_soft @ u1 <= b1_soft + cp.multiply(alpha_soft, h) + slack_constraints_soft, \
          A1_clf @ u1 <= b1_clf + slack_constraints_clf, cp.norm2(u1) <= U_max,
          alpha_soft >= np.zeros((num_constraints_soft1,1))]
objective1 = cp.Minimize( cp.sum_squares( u1 - u1_ref ) + 1000*cp.sum_squares(slack_constraints_clf) 
